...
class RoboticDatasetJoint(JointsDataset):
    def __init__(self, cfg, root, image_set, is_train, transform=None, task_name="none"):
        ...
        for video_dir in self.video_dirs:
            img_files = [os.path.join(video_dir, f) for f in os.listdir(video_dir) if (os.path.isfile(os.path.join(video_dir, f)) and f.endswith('.png'))]
            img_files = natsorted(img_files)
            img_files = img_files[:self.video_length_limit]
            self.video_img_dirs.append(img_files)
            obs = pickle.load(open(os.path.join(video_dir, 'obs.pkl'), 'rb'))
            self.video_obs.append(obs)
    ...
    def __getitem__(self, index):
        ...
        return img, target, target_weight, meta, obs_src, obs_tgt
